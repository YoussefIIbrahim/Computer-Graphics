...
from ClippingFilling.SLGraphic import SLGraphicsScene
from ClippingFilling.mathmodule import *
...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle('Clipping - Cyrus Beck')
        ...
        self.fill_butt = QPushButton('Apply', self)
        ...
        self.rb1.setText("Input lines")

        self.rb2 = QRadioButton(self)
        self.rb2.setText("Enter convex shutter")
        ...
        self.tab = QTabWidget()
        self.tab.addTab(self.infoTable, "Cutter")
        self.tab.addTab(self.table, "Pieces")
    ...
    def Cut(self):
        ...
        if len(self.edges) == 0:
            self.ShowWarning("No cut-off set!")
            return
        elif len(self.edges_slave) == 0:
            self.ShowWarning("No segment specified!")
            return

        convex_flag = check_convex(self.edges)
        if convex_flag == 0:
            self.ShowWarning("The given polygon is degenerate into a segment!")
            return
        # elif convex_flag == -1:
        #     self.ShowWarning("A non-convex polygon is specified!")
        #     return
        self.cut_convex(self.edges_slave, self.edges)

    def ShowDialog(self):
        if self.rb2.isChecked():
            text, ok = QInputDialog.getText(self, 'Input vertices',
                                            'Enter by type X Y:')
            if self.cutter_flag:
                self.edges.clear()
            ...
            text, ok = QInputDialog.getText(self, 'Input line',
                                            'Enter by type X1 Y1 X2 Y2:')
            if ok:
                text = text.split()
                x1 = int(text[0])
                y1 = int(text[1])
